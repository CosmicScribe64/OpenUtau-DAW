...
void verifyHostCallbackMatrix(juce::VST3PluginFormat& format,
                              ...
                              const juce::MemoryBlock& ustx) {
  // Exercise the sample rates and block sizes most likely to expose callback
  // slicing, transport, and sample-rate conversion bugs.
  ...
  // Use a window long enough to distinguish one 8,192-frame refill from
  // sustained callback starvation on a busy worker.
  ...
  for (const auto sampleRate : sampleRates) {
    ...
    for (const auto blockSize : blockSizes) {
      ...
      // Allow several render-ahead slices to fill before starting transport.
...
int main(int argc, char** argv) {
  try {
    ...
    juce::String error;
    // FL Studio can request 2,048 frames while the processor works in
    // 256-frame slices.
    ...
    juce::MidiBuffer midi;
    // Prewarm both instances while transport is stopped.
    ...
    // Check the render-ahead path used during live playback.
    playHead.playing = true;
    ...
    // Compare the live output with a direct render to catch dropped slices.
    ...
    // FL Studio re-prepares the processor when switching to a bounce. Keep the
    // existing sidecar alive across that transition.
    instance->releaseResources();
    instance->prepareToPlay(48000.0, hostBlockFrames);
    ...
    // FL Studio may report a stopped playhead during a non-realtime bounce.
    playHead.playing = false;
    bool heardOffline = false;
    deadline = std::chrono::steady_clock::now() + std::chrono::seconds(30);
    // Run a two-second bounce to cover consecutive offline blocks.
